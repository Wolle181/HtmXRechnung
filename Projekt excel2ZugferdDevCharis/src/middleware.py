...
erstellen, da ein Problem aufgetreten ist.\n{ex}",
    ...
    def _extract_xml_from_pdf(self, pdf_path: str) -> None:
        """Extrahiert das eingebettete ZUGFeRD-XML neben die PDF (still bei Fehler)."""
        try:
            from src.extract_xml_from_pdf import extract_xml
            extract_xml(pdf_path)
        except Exception as ex:
            self.logger.warning(f"XML-Extraktion fehlgeschlagen: {ex}")
    ...
    def fill_pdf(self, filename: str = None) -> None:
        ...
        if self.invoiceCollection.management.create_xml:
            if self._create_and_add_xml(fn, outfile):
                return
            self._success_message(outfile)
            self._extract_xml_from_pdf(outfile)
        else:
    ...
    def quiet_workflow(
        ...
        try:
            ...
            self._error_msg("Fehler beim Laden der Excel-Datei:", str(ex))
            return
