...
xmlns:udt="urn:un:unece:uncefact:data:standard:UnqualifiedDataType:100">\
  <rsm:ExchangedDocumentContext>\
    ...
    def test_fillPosAndNameOfLi_sets_name_and_description(self):
        """test that SpecifiedTradeProduct Name and Description are set"""
        from drafthorse.models.tradelines import LineItem

        li = LineItem()
        self.zugferd._fillPosAndNameOfLi(li, ["1", "2024-01-01 00:00:00", "Task A", "1", "h", "22", "22"])

        self.assertEqual(str(li.product.name), "Task A")
        self.assertEqual(str(li.product.description), "2024-01-01 00:00:00")

    def test_fillPosAndNameOfLi_name_is_two_line(self):
        """test that SpecifiedTradeProduct Name becomes two-line for long text"""
        from drafthorse.models.tradelines import LineItem

        long_task = "Dies ist ein sehr langer Text, der in zwei Zeilen dargestellt werden soll" 

        li = LineItem()
        self.zugferd._fillPosAndNameOfLi(li, ["1", "2024-01-01 00:00:00", long_task, "1", "h", "22", "22"])
