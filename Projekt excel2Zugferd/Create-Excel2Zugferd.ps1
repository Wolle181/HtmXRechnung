...
# =============================================================================
# VBA-Code: aus vba_src\Excel2ZugferdMakro.bas lesen (Single Source of Truth)
# =============================================================================
$vbaSrcPath = Join-Path $PSScriptRoot "vba_src\Excel2ZugferdMakro.bas"
if (-not (Test-Path $vbaSrcPath)) {
    Write-Error "VBA-Quelldatei nicht gefunden: $vbaSrcPath"
    exit 1
}
# Erste Zeile ('Attribute VB_Name ...) ist nur fuer den VBA-IDE-Export, wird weggelassen
$VBACode = (Get-Content -Path $vbaSrcPath -Raw -Encoding UTF8) -replace "(?m)^'Attribute VB_Name[^\r\n]*[\r\n]+", ""
Write-Host "    VBA-Code aus '$vbaSrcPath' geladen." -ForegroundColor Green
