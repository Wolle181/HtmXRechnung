...
class PDF(FPDF):
    ...
    def _printTableHeader(self):
        """set table Header"""
        self._set_section("Rechnungspositionen", 25, 135, "", 10)
        if self.table_lines:
    ...
    def _format_table_headers(self, arr: list) -> None:
        ...
        col_d = str(header[3]).strip()
        if "Gebührensatz" in col_d:
            header[3] = "Gebühren-\nsatz"
        elif "Arbeitnehmer" in col_d:
            header[3] = "Arbeit-\nnehmer"
        # Spalte F (Index 5) und G (Index 6): "in €" als zweite Zeile
        header[5] = header[5] + "\nin €"
        header[6] = header[6] + "\nin €"
    ...
    def _getSummenColWidths(self) -> tuple:
        return self.sum_table_widths if self.sum_table_widths else (58, 19)

    def _getSummenTableWidth(self) -> int:
        return sum(self.sum_table_widths) if self.sum_table_widths else 77
...
u.a. Konto."
    ...
    def fill_pdf(self, invoice: InvoiceCollection) -> None:
        ...
        # Leistungszeitraum hinzufügen (wenn beide Daten vorhanden)
        if von and bis:
