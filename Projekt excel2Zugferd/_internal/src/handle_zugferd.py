...
class ZugFeRD:
 ...
 verfügbar und wurde durch 'C62' (Stück) ersetzt."  # BT-154
    ...
    def add_items(self, dat, the_tax: str):
        ...
        for i, item in enumerate(dat):
            if i > 0:
                ...
                try:
                    menge = float(str(item[3]).split()[0].replace(",", ".")) if item[3] else 1.0
    ...
    def add_rechnungsperiode(self, posDatum: pd.DataFrame = None) -> None:
        if posDatum is None or len(posDatum) == 0:
            return
        
        posDatum = posDatum.copy()
        
        # Convert all to datetime first, coerce invalid values to NaT
        ...
        startDatum = posDatum.min(skipna=True)
        endDatum = posDatum.max(skipna=True)
        
        # Ensure startDatum and endDatum are valid datetime objects
        if pd.isna(startDatum) or pd.isna(endDatum):
            return
        
        try:
            self.doc.trade.delivery.event.occurrence = startDatum.to_pydatetime() if hasattr(startDatum, 'to_pydatetime') else startDatum
        except (AttributeError, TypeError):
            pass
        
        try:
            # BG-14 - Rechnungszeitraum
            self.doc.trade.settlement.period.start = startDatum.to_pydatetime() if hasattr(startDatum, 'to_pydatetime') else startDatum  # BT-73
            self.doc.trade.settlement.period.end = endDatum.to_pydatetime() if hasattr(endDatum, 'to_pydatetime') else endDatum  # BT-74
        except (AttributeError, TypeError):
            pass
