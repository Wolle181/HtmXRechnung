        ...
        try:
            ...
            if not items:
                messagebox.showwarning(
                    "Keine Tabellenblätter gefunden",
                    "Die Excel-Datei enthält keine Tabellenblätter oder kann nicht gelesen werden.",
                )
                return []
            for item in items:
            ...
            messagebox.showerror(
                ...
                f"Excel-Datei '{self.filename}' konnte nicht geladen werden.\n{ex}",
            )
            self.file_name_label.config(text="Bitte erst die Excel Datei auswählen (über Datei -> Öffnen...).")
            self.lb.delete(0, "end")
            return []
